...
(setq org-capture-templates
      '(("w" "Work")
        ("wt" "Work TODO" entry (file+headline "~/org/work.org" "Unsorted")
         (file "~/org/templates/work-todo.org"))
        ("h" "Home")
...
(defun jt/org-agenda-add-header ()
  ...
  (setq header-line-format
        "  n/p: next/prev  t: TODO  RET: goto  f/b: fwd/back  r: refresh  d: pushback  S: save  C: clear  q: quit"))
(add-hook 'org-agenda-mode-hook #'jt/org-agenda-add-header)

(defvar jt/org-agenda-pending-pushbacks nil
  "List of (marker . original-deadline) for pending pushbacks.")

(defun jt/org-agenda-pushback-deadline ()
  "Mark item at point to push back deadline by one day. Press S to save all."
  (interactive)
  (let ((marker (org-get-at-bol 'org-marker)))
    (when marker
      (push (cons (copy-marker marker) 1) jt/org-agenda-pending-pushbacks)
      (let ((inhibit-read-only t))
        (beginning-of-line)
        (when (re-search-forward "Deadline:" (line-end-position) t)
...
(defun jt/org-agenda-flush ()
  ...
  (org-agenda-redo)
  (message "Applied %d pushbacks" (length jt/org-agenda-pending-pushbacks)))
...
(defun jt/--open-daily-delayed ()
  (when (and jt/--pending-daily-frame (frame-live-p jt/--pending-daily-frame))
    (with-selected-frame jt/--pending-daily-frame
      ...
      (other-window 1)
      (org-agenda-list nil)
      (jt/agenda-scroll-to-current-time)
      (balance-windows)
      (other-window 1)))
  (setq jt/--pending-daily-frame nil))

(defun jt/agenda-scroll-to-current-time ()
  "Scroll agenda so that now line is visible with some context above."
  (goto-char (point-min))
  (when (re-search-forward "now - - -" nil t)
    (beginning-of-line)
    (recenter 1)))
        ...
        (cond
         ...
         (t
          (let ((event-name current-line))
            (when (and current-time current-month current-day)
              (with-current-buffer org-buffer
