...
#+begin_src elisp
    ...
    (defun jt/theme-changer-cleanup (_theme)
      "Ensure only one theme is enabled after theme change."
      (when (> (length custom-enabled-themes) 1)
        (dolist (theme (cdr custom-enabled-themes))
          (disable-theme theme))))

    (add-hook 'theme-changer-post-change-hook #'jt/theme-changer-cleanup)

    ;; Fix: suppress frame-set-background-mode during theme switch to prevent
    ;; O(faces² × frames) cascade. Changing the default face's :background
    ;; triggers frame-set-background-mode which recalculates ALL faces on
    ;; every frame — before enable-theme even finishes its own iteration.
    (advice-add 'theme-changer-switch-theme :around
      (lambda (orig-fn old new)
        (let ((inhibit-redisplay t))
          (unwind-protect
              (progn
                (advice-add 'frame-set-background-mode :override #'ignore)
                (funcall orig-fn old new))
            (advice-remove 'frame-set-background-mode #'ignore)
            (dolist (frame (frame-list))
              (frame-set-background-mode frame))))))

    ;; Fix: each change-theme call creates a run-at-time timer. Multiple
    ;; calls (from server-after-make-frame-hook) accumulate timers that all
    ;; fire simultaneously after suspend/resume.
    (defvar jt/theme-changer-timer nil)
    (advice-add 'change-theme :around
      (lambda (orig-fn &rest args)
        (when (timerp jt/theme-changer-timer)
          (cancel-timer jt/theme-changer-timer))
        (apply orig-fn args)
        (setq jt/theme-changer-timer
              (seq-find (lambda (timer)
                          (eq (timer--function timer) 'change-theme))
                        timer-list))))
...
** sidekick buffer (always light)
A buffer that always renders with spacemacs-light, regardless of the
globally-active theme. Implemented with buffer-local face remapping so it
doesn't perturb other buffers/frames. Launch with:
#+begin_src elisp
(defvar jt/sidekick-light-faces nil
  "Alist of (FACE . ATTRS) captured from spacemacs-light, used by face-remap.")

(defun jt/sidekick-capture-light-faces ()
  "Load spacemacs-light without enabling it and snapshot its face specs."
  (load-theme 'spacemacs-light t t)
  (let (acc)
    (dolist (s (get 'spacemacs-light 'theme-settings))
      (when (and (consp s) (eq (car s) 'theme-face))
        (let* ((face (nth 1 s))
               (raw  (nth 3 s))
               (spec (if (and (consp raw) (eq (car raw) 'quote)) (cadr raw) raw))
               (attrs (ignore-errors (face-spec-choose spec))))
          (when (and (listp attrs) (keywordp (car-safe attrs)))
            (push (cons face attrs) acc)))))
    (setq jt/sidekick-light-faces (nreverse acc))))

(defun jt/sidekick-apply-light ()
  "Remap faces in the current buffer to the captured spacemacs-light specs."
  (unless jt/sidekick-light-faces
    (ignore-errors (jt/sidekick-capture-light-faces)))
  (dolist (entry jt/sidekick-light-faces)
    (ignore-errors
      (face-remap-add-relative (car entry) (cdr entry)))))

(defun jt/sidekick-buffer ()
  "Get or create *sidekick*, with spacemacs-light face remap applied."
  (let ((buf (get-buffer-create "*sidekick*")))
    (with-current-buffer buf
      (unless (bound-and-true-p jt/sidekick-applied)
        (setq-local jt/sidekick-applied t)
        (jt/sidekick-apply-light)
        ;; :height 636 yields 28 cols x 10 lines in the SidekickEmacs frame
        ;; at its current pixel size; next step (637) drops to 9 lines.
        (face-remap-add-relative 'default :height 636)
        (setq-local mode-line-format nil)
        (setq-local left-fringe-width 0)
        (setq-local right-fringe-width 0))
      ;; Re-asserted every call: global-display-line-numbers-mode toggles
      ;; on config reload and re-enables despite -set-explicitly.
      (when (fboundp 'display-line-numbers-mode)
        (display-line-numbers-mode -1))
      (setq-local display-line-numbers-mode-set-explicitly t)
      (setq-local display-line-numbers nil))
    ;; Ensure enough lines exist for col/row guides to render visibly.
    (with-current-buffer buf
      (when (= (buffer-size) 0)
        (insert (make-string 30 ?\n))
        (goto-char (point-min))))
    (unless (get-buffer "*sidekick-dark*")
      ;; Indirect buffer: shares text with *sidekick* (edits propagate
      ;; both ways) but has its own buffer-local settings, so the dark
      ;; twin renders normally.
      (let ((dark (make-indirect-buffer buf "*sidekick-dark*" nil)))
        (with-current-buffer dark
          (jt/sidekick-dark-setup-bounds))))
    buf))

(defvar-local jt/sidekick-overflow-ov nil)

(defun jt/sidekick-dark-update-overflow (&rest _)
  "Underline line 10 in *sidekick-dark* to mark the bottom of the usable area."
  (when (eq (current-buffer) (get-buffer "*sidekick-dark*"))
    (let* ((line-10-beg (save-excursion (goto-char (point-min))
                                        (forward-line 9) (point)))
           (line-10-end (save-excursion (goto-char line-10-beg)
                                        (line-end-position))))
      (unless jt/sidekick-overflow-ov
        (setq jt/sidekick-overflow-ov (make-overlay 1 1 nil nil t))
        (overlay-put jt/sidekick-overflow-ov 'face
                     '(:underline (:color "gray" :position t) :extend t)))
      (move-overlay jt/sidekick-overflow-ov line-10-beg (1+ line-10-end)))))

(defun jt/sidekick-dark-setup-bounds ()
  "Mark the usable 28x10 region inside *sidekick-dark* visually."
  ;; Vertical bar at col 29 (usable cols are 1..28).
  (setq-local fill-column 28)
  (setq-local display-fill-column-indicator-character ?│)
  (face-remap-add-relative 'fill-column-indicator '(:foreground "gray"))
  (when (fboundp 'display-fill-column-indicator-mode)
    (display-fill-column-indicator-mode 1))
  ;; Horizontal overflow tint past line 10.
  (add-hook 'after-change-functions
            #'jt/sidekick-dark-update-overflow nil t)
  (jt/sidekick-dark-update-overflow))

;; Defensive: if anything turns line numbers back on in *sidekick*, kick it off.
(add-hook 'display-line-numbers-mode-hook
          (lambda ()
            (when (and display-line-numbers-mode
                       (equal (buffer-name) "*sidekick*"))
              (display-line-numbers-mode -1))))

(defun jt/show-sidekick ()
  "Open (or raise) the SidekickEmacs frame showing the *sidekick* buffer.
Frame has no minibuffer (it shares the default frame's)."
  (interactive)
  (let* ((existing (cl-find-if (lambda (f)
                                 (equal (frame-parameter f 'name) "SidekickEmacs"))
                               (frame-list)))
         (frame (or existing
                    (make-frame '((name . "SidekickEmacs")
                                  (minibuffer . nil))))))
    (select-frame-set-input-focus frame)
    (with-selected-frame frame
      (switch-to-buffer (jt/sidekick-buffer))
...
(defvar jt/no-whitespace-modes '(eat-mode)
  "Modes where whitespace display should be disabled.")

(defun jt/disable-whitespace-display ()
  "Disable whitespace display for terminal and special buffers."
  (when (or (apply #'derived-mode-p jt/no-whitespace-modes)
            (string-prefix-p "*claude-code" (buffer-name)))
    (setq-local show-trailing-whitespace nil)
    (whitespace-mode -1)))

(add-hook 'after-change-major-mode-hook #'jt/disable-whitespace-display)
#+end_src
...
(defun jt/org-agenda-add-header ()
  ...
  (setq header-line-format
        "  n/p: next/prev  t: TODO  RET: goto  f/b: fwd/back  .: today  r: refresh  d: pushback  S: save  C: clear  G: gcal  h/w/F: org files  q: quit")
  (setq-local show-trailing-whitespace nil))
...
(with-eval-after-load 'org-agenda
  ...
  (define-key org-agenda-mode-map (kbd "G") #'jt/gcal-import))
...
*** Org link display
#+begin_src elisp
;; @jt-menu View | l | Toggle link display | org-toggle-link-display
(define-key org-mode-map (kbd "C-c C-M-l") 'org-toggle-link-display)
#+end_src
*** Refresh link title from URL
#+begin_src elisp
;; @jt-menu Edit | r | Refresh link title | jt/org-link-refresh-title
(defun jt/org-link-refresh-title ()
  "Fetch the current page title from the URL at point and update the link."
  (interactive)
  (let* ((context (org-element-context))
    ...
    (let* ((link-type (org-element-property :type context))
           ...
           (url (if (member link-type '("http" "https"))
                    (concat link-type ":" path)
                  (user-error "Not an HTTP(S) link")))
           (begin (org-element-property :begin context))
           (end (org-element-property :end context))
           (trailing-space (when (save-excursion
                                   (goto-char (1- end))
                                   (looking-at-p " "))
                             " ")))
      (message "Fetching title from %s..." url)
      (let ((title (jt/fetch-url-title url)))
        (if title
            (progn
              (setq title (replace-regexp-in-string "\\[\\|\\]" "" title))
              (delete-region begin end)
              (insert (format "[[%s][%s]]%s" url title (or trailing-space "")))
              (message "Updated title: %s" title))
          (message "Could not fetch title"))))))

(defun jt/fetch-url-title (url)
  "Fetch URL and extract the <title> tag contents."
  (with-temp-buffer
    (condition-case err
        (progn
          (call-process "curl" nil t nil "-sL" "-m" "10"
                        "-H" "User-Agent: Mozilla/5.0" url)
          (goto-char (point-min))
          (when (re-search-forward "<title[^>]*>\\([^<]+\\)</title>" nil t)
            (string-trim (match-string 1))))
      (error
       (message "Error fetching URL: %s" err)
       nil))))

(define-key org-mode-map (kbd "C-c r") 'jt/org-link-refresh-title)

;; @jt-menu Edit | R | Refresh links by domain | jt/org-links-refresh-by-domain
(defun jt/org-links-refresh-by-domain (domain)
  "Refresh titles for all links matching DOMAIN in current buffer.
Pauses 2 seconds between requests to avoid overloading servers."
  (interactive "sRefresh links matching domain: ")
  (let ((links '())
        (processed 0)
        (failed 0))
      ...
      (lambda (link)
        (let* ((type (org-element-property :type link))
          ...
          (when (and (member type '("http" "https"))
                     (string-match-p (regexp-quote domain) path))
    ...
    (if (null links)
        (message "No links matching '%s' found" domain)
      (message "Found %d links matching '%s'" (length links) domain)
      (dolist (link links)
        (let* ((url (plist-get link :url))
               (title (jt/fetch-url-title url)))
          (cl-incf processed)
          (if title
              (progn
                (setq title (replace-regexp-in-string "\\[\\|\\]" "" title))
                ...
                (message "[%d/%d] Updated: %s"
                         processed (length links)
                         (truncate-string-to-width title 50 nil nil "…")))
            (cl-incf failed)
            (message "[%d/%d] Failed: %s" processed (length links) url))
          (when (< processed (length links))
            (sleep-for 2))))
      (message "Done. Updated %d/%d links (%d failed)"
               (- processed failed) (length links) failed))))

(define-key org-mode-map (kbd "C-c M-r") 'jt/org-links-refresh-by-domain)
#+end_src
** Which key
#+begin_src elisp
...
;; @jt-menu View | # | Line numbers | display-line-numbers-mode
...
(defun jt/--open-daily ()
  "Open today's org-roam daily and agenda in new emacsclient frames.
Only runs when the frame would show *scratch* (no file argument)."
...
(when (daemonp)
  (add-hook 'server-after-make-frame-hook #'jt/--open-daily))
#+end_src
** Org-Inkscape
#+begin_src elisp
...
** Countdown timer
#+begin_src elisp
;; @jt-menu Tools | t c | Countdown timer | jt/countdown-timer
(defvar jt/countdown-timer nil
  "Timer object for the countdown.")

(defvar jt/countdown-remaining 0
  "Remaining seconds in countdown.")

(defvar jt/countdown-buffer-name "*Countdown*"
  "Name of the countdown buffer.")

(defvar jt/countdown-start-time nil
  "Time when countdown was started.")

(defvar jt/countdown-minutes nil
  "Number of minutes for the countdown.")

(defun jt/countdown-update ()
  "Update the countdown display."
  (when (> jt/countdown-remaining 0)
    (setq jt/countdown-remaining (1- jt/countdown-remaining))
    (let ((buf (get-buffer jt/countdown-buffer-name)))
      (when buf
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            (erase-buffer)
            (jt/countdown-insert-display))))))
  (when (<= jt/countdown-remaining 0)
    (jt/countdown-finished)))

(defun jt/countdown-insert-display ()
  "Insert the countdown display into current buffer."
  (let* ((mins (/ jt/countdown-remaining 60))
         (secs (mod jt/countdown-remaining 60))
         (time-str (format "%d:%02d" mins secs)))
    (insert (propertize time-str
                        'face '(:height 4.0 :weight bold)))
    (insert "\n\n")
    (insert (propertize (format "%dm \@ %s\n" jt/countdown-minutes jt/countdown-start-time)
                        'face '(:height 1.0 :foreground "gray50")))
    (insert (propertize "Press 'q' to cancel"
                        'face '(:height 1.0 :foreground "gray50")))))

(defun jt/countdown-finished ()
  "Handle countdown completion."
  (when jt/countdown-timer
    (cancel-timer jt/countdown-timer)
    (setq jt/countdown-timer nil))
  (let ((buf (get-buffer jt/countdown-buffer-name)))
    (when buf
      (with-current-buffer buf
        (let ((inhibit-read-only t))
          (erase-buffer)
          (insert (propertize "TIME!"
                              'face '(:height 4.0 :weight bold :foreground "red")))
          (insert "\n\n")
          (insert (propertize "Press 'q' to close"
                              'face '(:height 1.0 :foreground "gray50")))))))
  (start-process "countdown-bell" nil "paplay" "/usr/share/sounds/freedesktop/stereo/complete.oga"))

(defun jt/countdown-quit ()
  "Cancel countdown and close frame."
  (interactive)
  (when jt/countdown-timer
    (cancel-timer jt/countdown-timer)
    (setq jt/countdown-timer nil))
  (let ((buf (get-buffer jt/countdown-buffer-name)))
    (when buf
      (let ((frame (car (seq-filter
                         (lambda (f)
                           (eq buf (window-buffer (frame-selected-window f))))
                         (frame-list)))))
        (kill-buffer buf)
        (when (and frame (> (length (frame-list)) 1))
          (delete-frame frame))))))

(defun jt/countdown-timer (minutes)
  "Start a countdown timer for MINUTES in a new frame."
  (interactive "nMinutes: ")
  (jt/countdown-quit)
  (setq jt/countdown-start-time (format-time-string "%H:%M"))
  (setq jt/countdown-minutes minutes)
  (setq jt/countdown-remaining (* minutes 60))
  (let* ((frame (make-frame '((name . "Countdown")
                              (width . 60)
                              (height . 18))))
         (buf (get-buffer-create jt/countdown-buffer-name)))
    (select-frame frame)
    ...
    (with-current-buffer buf
      ...
      (local-set-key (kbd "q") #'jt/countdown-quit))
    (setq jt/countdown-timer
          (run-with-timer 1 1 #'jt/countdown-update))))
#+end_src
** browser integration
#+begin_src elisp
...
#+begin_src elisp
;; @jt-menu Tools | t b | Import Session Buddy JSON | jt/import-session-buddy
(defun jt/import-session-buddy (json-file target-file)
  "Import Session Buddy JSON export into org checkbox format.
JSON-FILE is the exported .json file from Session Buddy.
TARGET-FILE is the org file to append to (e.g. ~/org/fun.org)."
  (interactive
   (list (read-file-name "Session Buddy JSON file: " "~/Downloads/" nil t nil
                         (lambda (f) (or (file-directory-p f)
                                         (string-suffix-p ".json" f))))
         (expand-file-name
          (read-file-name "Target org file: " "~/org/" "fun.org" nil "fun.org"))))
  (setq target-file (expand-file-name target-file))
  (let* ((json-object-type 'alist)
         (json-array-type 'list)
         (json-key-type 'symbol)
         (data (json-read-file json-file))
         (collections (alist-get 'collections data))
         (total-tabs 0)
         (output "")
         (timestamp (format-time-string "[%Y-%m-%d %H:%M]")))
    (setq output (format "* Browser Import %s [/]\n" timestamp))
    (let ((window-num 1))
      (dolist (collection collections)
        (dolist (folder (alist-get 'folders collection))
          (let* ((links (alist-get 'links folder))
                 (last-link (car (last links)))
                 (window-hint (if last-link
                                  (truncate-string-to-width
                                   (or (alist-get 'title last-link)
                                       (alist-get 'url last-link) "?")
                                   40 nil nil "…")
                                "empty"))
                 (link-count (length links)))
            (setq output (concat output
                                 (format "** Window %d: %s [0/%d]\n"
                                         window-num window-hint link-count)))
            (dolist (link links)
              (let ((url (alist-get 'url link))
                    (title (or (alist-get 'title link) "")))
                (when url
                  (setq title (if (string-empty-p title)
                                  url
    ...
    (goto-char (point-max))
    (message "Imported %d tabs into %s" total-tabs target-file)))
#+end_src
** lang stuff
#+begin_src elisp
...
#+end_src
** Claude Code IDE
Integration with Claude Code CLI via MCP protocol.
#+begin_src elisp
;; @jt-menu AI | a c | Claude Code | claude-code-ide
;; @jt-menu AI | a m | Claude Menu | claude-code-ide-menu
;; @jt-menu AI | a s | Claude Sessions | claude-code-ide-list-sessions
(use-package eat :ensure t)

;; @jt-menu AI | a u | Copy visible with URLs | jt/copy-visible-with-unwrapped-urls
(defun jt/copy-visible-with-unwrapped-urls ()
  "Copy visible buffer contents to scratch buffer with URLs unwrapped.
Grabs a few extra lines beyond visible area to compensate for minibuffer."
  (interactive)
  (let* ((start (window-start))
         (end (save-excursion
                ...
                (forward-line 10)
                (point)))
         (content (buffer-substring-no-properties start (min end (point-max))))
         (buf (generate-new-buffer "*scratch-urls*")))
    (with-current-buffer buf
      (insert content)
      (goto-char (point-min))
      (let ((found t))
        (while found
          ...
          (while (re-search-forward "\\(https?://[^ \t\n]*\\)\n\\([^ \t\n]+\\)" nil t)
            (let ((before (match-string 1))
                  (after (match-string 2)))
              (when (and (not (string-match-p "^https?://" after))
                         (string-match-p "^[a-zA-Z0-9/_?.=&%-]" after))
                (replace-match (concat before after) t t)
                (setq found t))))))
      (goto-address-mode 1)
      (goto-char (point-min)))
          ...
          (let ((event-name (string-trim current-line)))
            (when (and current-time current-month current-day (not (string-empty-p event-name)))
              (let ((duration (jt/gcal-duration-hours current-time)))
                (unless (and (string-equal "XTDB Chat" event-name)
                             duration
                             (> duration 6))
                  (with-current-buffer org-buffer
...
Opens a buffer to paste calendar text, then writes to work-cal.org."
  ...
  (let ((raw-buffer (get-buffer-create "*jt/gcal-import-raw*")))
    ...
    (erase-buffer)
    (browse-url "https://calendar.google.com/calendar/u/1/r/agenda")
...
#+end_src
** org wishlist TODO
see parent org lines at top of the buffer (vscode extension does this with markdown? pretty cool)
