...
(defun jt/org-image-scale-refresh ()
  "Refresh inline images scaled to current text zoom level."
  (when (derived-mode-p 'org-mode)
    (let* ((scale (expt text-scale-mode-step text-scale-mode-amount))
           (org-image-actual-width (truncate (* 400 scale))))
      (org-display-inline-images t t))))

(add-hook 'text-scale-mode-hook #'jt/org-image-scale-refresh)
